...
// Any 1036/1038 occurrence (any flag) per section
for(const s of [1,4,10]){
  const start=secs[s],end=s+1<secs.length?secs[s+1]:data.length
  console.log('=== SEC',s,'===')
  for(let i=start;i<end-30;i++){const v=u32(i); if(v===1036||v===1038){console.log('  marker',v,'at +0..+16:',[0,4,8,12,16].map(o=>u32(i+o)).join(','))}}
}
