...
function strs(d,s,e){const o=[];let c=[];for(let i=s;i<e-1;i+=2){const lo=d[i],hi=d[i+1];const ok=hi===0&&((lo>=0x20&&lo<=0x7e));if(ok){c.push(String.fromCharCode(lo))}else{if(c.length>=2)o.push(c.join(''));c=[]}}return o}
...
console.log('AB17 doubles @1020..1100 (every 4 bytes):')
for(let o=1020;o<=1100;o+=4){const v=rd(start+o);if(Math.abs(v)<1e7&&Math.abs(v)>1e-6)console.log('  off',o,'=',v)}
