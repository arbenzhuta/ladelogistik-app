...
// Aligned doubles: report the clean [a,b,L] near the start (the canonical RECT-Straight triple)
for(const tgt of ['1.07_Z_ZU67','1.07_Z_ZU68','1.08_Z_ZU15*','1.07_Z_ZU64','1.07_Z_ZU72']){
  const s=secOf(tgt);const start=secs[s]
  // print aligned ints in window 1020..1260 on the grid that contains the triple
  const vals=[]
  for(let o=1020;o<=1280;o+=4){const v=rd(start+o);if(v>0&&v<10000&&v===Math.floor(v))vals.push(o+':'+v)}
  console.log(tgt, vals.join('  '))
}
